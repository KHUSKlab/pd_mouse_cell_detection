...
#% Default settings
    def setupUi(self, MainWindow):
        ...
        self.label_name.setGeometry(QtCore.QRect(18, 121, 501, 16))
...
#%% functions
    ...
    def loadImage(self): # image open
        ...
        self.file_now = self.name(self.filename)
        nm= self.filename.split('/')[-1]
        self.label_name.setText(nm)
        # enable and disable
        self.pushButton_crop.setEnabled(True)
        # enable
    ...
    def drawRect(self):
        ...
        cv2.moveWindow('DRAG to draw a rectangle. After selecting the area, PRESS ENTER to nail it down. PRESS ESC to quit without selecting.', 0,210)
        if w and h:
            roi = img[y:y+h, x:x+w]
...
# contrast
    ...
    def prediction(self):
        ...
        def ms_prep_and_predict(t_im=None, sh=14, \
            ...
            for div_i in range(divnum):
                print('div', div_i)
                self.progressBar.setValue(20+(div*5))
                QtWidgets.QApplication.processEvents()
        ...
        background_img[:,:30] = [255,255,255]
        background_img[:,-30:] = [255,255,255]
        background_img[:31,:] = [255,255,255]
        background_img[-30:,:] = [255,255,255]

        prediction_img[:,:30] = [0,0,0]
        prediction_img[:,-30:] = [0,0,0]
        prediction_img[:31,:] = [0,0,0]
        prediction_img[-30:,:] = [0,0,0]        
        ...
        if self.stop_waiting <2:
            ...
            for i in range(len(b)-1):
                ...
                if i == len(b)-1:
                    ...
                    c = c+'.'
            csvfn = c + '_crop.csv'
            filename = QFileDialog.getSaveFileName(directory=csvfn,filter="CSV(*.csv)")[0]
    ...
    def savePhoto(self):
        ...
        if str(type(self.tmp)) == "<class 'NoneType'>":
            ...
            filename = QFileDialog.getSaveFileName(directory=self.filename,filter="JPG(*.jpg);;PNG(*.png);;TIFF(*.tiff);;BMP(*.bmp)")[0]
        ...
        while True:
            ...
            def Mouse(event,x,y,flag,param):
                if event == cv2.EVENT_LBUTTONUP:
                    ...
                    self.winx = cv2.getWindowImageRect('')[0] -8
                    self.winy = cv2.getWindowImageRect('')[1] -8
            cv2.setMouseCallback('',Mouse)
