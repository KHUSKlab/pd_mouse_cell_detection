...
def get_F1(threshold=None, contour_thr=None,\
    ...
    msdict = {'los': los, 'co': co, 'tp': TP, 'fp': FP, 'fn': FN, \
              'predbox2': predbox2, 'ansbox': ansbox, 'pink': pink, 't_im': t_im}
...
mainpath = 'C:\\Temp\\cell_detection_en_revision\\20230209_en_revision_original_augmentation\\'
...
for n_num in range(len(nlist_for)):
    ...
    if c1 and c2:
        ...
        if False:
            ...
            sz = 5
            for j in range(len(pink)):
                tim[pink[j][1]-sz:pink[j][1]+sz, pink[j][0]-sz:pink[j][0]+sz, 1] = 255
                
            plt.imshow(np.mean(tim, axis=2))
            im_uint8 = Image.fromarray(np.uint8(tim))

        
        
        
        
        psave2 = weight_savepath + 'F1_msdict_' + msid + '.pickle'
...
excel_save = weight_savepath + 'F1score_result_thrfix.xls'
mssave2_df = pd.DataFrame(mssave2, columns=['ID', 'Human #', 'Model #', 'F1_score', 'TP', 'FP', 'FN', 'threshold', 'contour_thr'])
mssave2_df.to_excel(excel_save)
