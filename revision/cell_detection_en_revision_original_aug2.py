...
def get_F1(threshold=None, contour_thr=None,\
    ...
    dots = list(co)
    white3 = dot_expand(height=height, width=width, dots=dots)
    cell_positive_area = []
    w = np.where(white3[:,:,0]==0)
    for i in range(len(w[0])):
...
for n_num in range(len(tnlist)):
    ...
    if msid == '15.tif_resize_crop':
        path6 = 'C:\\SynologyDrive\\study\\dy\\TH_안소라선생님\\roi info\\'
        filename = msid[:-16] + '_CellCounter.csv'
        df2 = pd.read_csv(path6 + filename)
        markerix = np.where(np.array(df2)[:,0] == 'marker x')[0][0]
        roiix = np.where(np.array(df2)[:,0] == 'roi x')[0][0]
        
        rois = np.array(df2.iloc[markerix+1:roiix,:], dtype=int)
        marker_x = np.array(rois)[:,0]
        marker_y = np.array(rois)[:,1]
    ...
    rois = np.array(df2.iloc[roiix+1:,:], dtype=float)
    points = np.array(np.round(rois), dtype=int)
    polygon = Polygon(points)
    

    #%
    msdict = {'t_im': np.array(im), 'marker_x': marker_x, \
              'marker_y': marker_y, 'polygon': polygon, 'points': points, 'positive_indexs': positive_indexs}
...
excel_save = weight_savepath + 'F1score_result_augmodel_anr_roi.xls'
print(excel_save)
mssave2_df = pd.DataFrame(np.array(mssave2[:,1:], dtype=float), \
             columns=['Human #', 'Model #', 'F1_score', 'TP', 'FP', 'FN', 'threshold', 'contour_thr'], \
             index=mssave2[:,0])
