...
mainpath = 'C:\\SynologyDrive\\study\\dy\\image_data\\unprocessed_imgs\\org\\'
mainpath_common = 'C:\\SynologyDrive\\study\\dy\\image_data\\'
psave = mainpath + 'data_nonprocess_ms.pickle'
...
def msXYZgen(im=None, marker_x=None, marker_y=None):
    ...
    im_gray = np.mean(im, axis=2) # 3d -> 2d
    im_uint8 = Image.fromarray(np.uint8(im_gray))
    various_saves = []
    var_save = []
    for bright_var in np.arange(0.5, 3, 0.2):
        en = ImageEnhance.Contrast(im_uint8).enhance(bright_var)
        for con_var in np.arange(1.5, 4, 0.5):
            var_save.append([bright_var, con_var])
        
            enh = ImageEnhance.Contrast(en).enhance(con_var)
            enh = np.array(enh)
            # plt.figure(); plt.imshow(enh)
            
            various_saves.append(enh)

    various_i = 0
    xtmp_save = msFunction.msarray([len(various_saves)])
    for various_i in range(len(various_saves)):
        timg = various_saves[various_i]
        im_padding = np.ones((im_gray.shape[0]+SIZE_HF*2, im_gray.shape[1]+SIZE_HF*2)) * 255
        im_padding[SIZE_HF:-SIZE_HF, SIZE_HF:-SIZE_HF] = timg
        ...
        for i in range(len(marker_x)):
            ...
            if crop.shape == (SIZE_HF*2, SIZE_HF*2):
                crop = crop / np.mean(crop)
                if not(np.isnan(np.mean(crop))):
                    xtmp_save[various_i].append(np.array(crop))
    ...
    for various_i in range(len(various_saves)):
        savetmp = []
        for j in range(len(xtmp_save[various_i])):
            
            mask1 = np.array(mask)
            mask1[np.where(mask1==0)] = np.nan
            mask2 = np.array(mask)
            mask2[np.where(mask2==1)] = np.nan
            mask2[np.where(mask2==0)] = 1
            
            # plt.imshow(mask1)
            # plt.imshow(mask2)
            
            center = np.array(xtmp_save[various_i][j]) * mask1
            peripheri = np.array(xtmp_save[various_i][j]) * mask2

            # plt.imshow(center)
            # plt.imshow(peripheri)
            savetmp.append(np.nanmedian(peripheri) - np.nanmedian(center))
            
        std_saves.append(np.sum(savetmp))
    
    mix = np.argmax(std_saves)
    
    ##
    
    print('brightness', var_save[mix][0], 'contrast', var_save[mix][1])

    im_uint8_3ch = Image.fromarray(np.uint8(im))
    
    en = ImageEnhance.Contrast(im_uint8_3ch).enhance(var_save[mix][0])
    enh = ImageEnhance.Contrast(en).enhance(var_save[mix][1])          
    enh = np.array(enh)
    im_padding = np.ones((enh.shape[0]+SIZE_HF*2, enh.shape[1]+SIZE_HF*2, 3)) * 255
    im_padding[SIZE_HF:-SIZE_HF, SIZE_HF:-SIZE_HF, :] = enh
    ...
    for i in range(len(marker_x)):
        ...
        for ei in range(len(rowcol_list)):
            ...
            if crop.shape == (SIZE_HF*2, SIZE_HF*2, 3) and not(np.isnan(np.mean(crop))):
                for ch in range(3):
        ...
        rotatelist = list(range(0, 360-1, 45))
        for degree in rotatelist:
            large_r = int((SIZE_HF * np.sqrt(2)) + 1)
            background = im_padding[row-large_r:row+large_r, col-large_r:col+large_r, :]
            background_2 = Image.fromarray(np.array(background, dtype='uint8'))
            rotated = background_2.rotate(degree)
            
            rotated2 = np.array(rotated)[large_r-SIZE_HF:large_r+SIZE_HF, large_r-SIZE_HF:large_r+SIZE_HF]
            crop = np.array(rotated2)
            if crop.shape == (SIZE_HF*2, SIZE_HF*2, 3) and not(np.isnan(np.mean(crop))):
                for ch in range(3):
                    crop[:,:,ch] = crop[:,:,ch]/np.mean(crop[:,:,ch])
                # crop = np.reshape(crop, (crop.shape[0], crop.shape[1], 1))
...
mainpath = 'C:\\Temp\\cell_detection_en_revision\\20230209_en_revision_original_multi_input\\'
...
for cv in range(len(cvlist)): # range(len(cvlist)):
    ...
    for n_num in telist:
        ...
        #% pre processing random으로 결정
        
        tlist = list(range(len(marker_x)))
        random.seed(1)
        rlist = random.sample(tlist, 10)
