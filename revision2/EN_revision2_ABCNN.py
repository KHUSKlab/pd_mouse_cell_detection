...
# mainpath = 'C:\\Temp\\cell_detection_en_revision\\20230209_en_revision_original\\'
mainpath = 'F:\\cell_detection_en_revision\\20230209_en_revision_original\\'
...
weight_savepath = mainpath + 'weightsave_ABCNN_' + str(lnn) + '\\'; msFunction.createFolder(weight_savepath)
...
# for cv in range(0, len(cvlist)):
    ...
    if not(os.path.isfile(final_weightsave)) or False:
        ...
        model = model_setup_ABCNN(xs=(28, 28, 3), ys=2, lnn=lnn)
...
for n_num in range(len(nlist_for)):
    ...
    if (not(os.path.isfile(psave2)) or False) and os.path.isfile(psave):
        with open(psave, 'rb') as file:
            msdict = pickle.load(file)
...
excel_save = weight_savepath + 'F1score_result_ABCNN.xls'
print(excel_save)
